...
const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col gap-6 no-print">
          ...
          <div className="flex flex-col gap-1 w-full xl:max-w-xl">
            <div className="relative flex items-center bg-slate-50 border rounded-2xl p-1">
              ...
              <input
                ...
                className="flex-1 py-4 text-sm font-bold bg-transparent outline-none uppercase"
              />
            </div>
            {lastSynced && (
          ...
          <div className="flex flex-wrap items-center gap-4">
              ...
              <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                {[
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${filterMode === mode.id ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {mode.label}
                  </button>
                ))}
            ...
            <div className="flex flex-col gap-1">
              ...
              <select
                ...
                className="bg-slate-100 border border-slate-200 rounded-xl px-4 py-2 text-[9px] font-black uppercase tracking-widest outline-none focus:border-indigo-600 appearance-none cursor-pointer h-[38px] min-w-[120px]"
              >
      ...
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden no-print">
                ...
                <td className="px-10 py-7">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <h5 className="font-black text-slate-900 text-sm uppercase mt-1">{item.brand} {item.name}</h5>
                      <p className="text-[10px] text-slate-400 font-mono mt-0.5">{item.barcode}</p>
                    </div>
                  </div>
                </td>
                <td className="px-10 py-7 text-center">
                  <span className={`text-xl font-black font-mono ${item.stock <= item.minStock ? 'text-rose-500' : 'text-slate-900'}`}>{item.stock}</span>
                </td>
                <td className="px-10 py-7 text-center">
                  <p className="text-lg font-black font-mono">{SHOP_INFO.currency}{(item.price || 0).toFixed(2)}</p>
                </td>
                <td className="px-10 py-7 text-right">
                  <button onClick={() => setEditingItem({ ...item })} className="p-3 bg-indigo-50 rounded-xl text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all">✎</button>
                </td>
              </tr>
            )) : (
              <tr>
        ...
        <div className="md:hidden p-4 space-y-4 bg-slate-50">
          {filteredInventory.length > 0 ? filteredInventory.map(item => (
            <div key={item.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col gap-4">
      ...
      {editingItem && (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-xl flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-5xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col h-[90vh]">
            <div className="bg-indigo-600 p-10 text-white flex justify-between items-center shrink-0 shadow-lg">
              <div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-4 text-xs font-bold outline-none focus:border-indigo-600 transition-all"
                    />
                  </div>
                </div>
                <div className="space-y-8">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input type="text" value={editingItem.brand} onChange={e => setEditingItem({ ...editingItem, brand: e.target.value.toUpperCase() })} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black uppercase text-sm outline-none focus:border-indigo-600" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Asset Name</label>
                      <input type="text" value={editingItem.name} onChange={e => setEditingItem({ ...editingItem, name: e.target.value.toUpperCase() })} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black uppercase text-sm outline-none focus:border-indigo-600" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Stock Quantifier</label>
                      <input type="number" value={editingItem.stock} onChange={e => setEditingItem({ ...editingItem, stock: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black font-mono text-lg outline-none focus:border-indigo-600" />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input type="number" step="0.01" value={editingItem.price} onChange={e => setEditingItem({ ...editingItem, price: parseFloat(e.target.value) || 0 })} className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black font-mono text-lg text-emerald-600 outline-none focus:border-indigo-600" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        className="w-full bg-slate-50 border border-slate-200 p-5 rounded-xl font-black uppercase text-sm outline-none focus:border-indigo-600 appearance-none cursor-pointer"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black uppercase text-sm outline-none focus:border-indigo-600"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl font-black font-mono text-sm outline-none focus:border-indigo-600"
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6 pt-4">
                    ...
                    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex flex-col gap-1">
                      ...
                      <span className="text-sm font-black text-slate-900 font-mono">{editingItem.sku || 'PENDING SYNC'}</span>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="p-10 bg-slate-50 border-t flex justify-end gap-6 shrink-0 shadow-inner">
              <button
                onClick={handleSave}
                disabled={isSaving}
